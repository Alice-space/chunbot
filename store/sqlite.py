...
class SQLiteStore(Store):
    ...
    def update_list(
        ...
        # Get existing titles
        cursor.execute(
            "SELECT title FROM news WHERE description = ?", (source_description,)
        )
        existing_titles = {row[0] for row in cursor.fetchall()}

        # Find new entries
        new_entries = {
            title: url for title, url in news.items() if title not in existing_titles
        }
        ...
        if new_entries:
            ...
            cursor.executemany(
                ...
                [
                    (source_description, title, url, local_time)
                    for title, url in new_entries.items()
                ],
            )
            self.conn.commit()
